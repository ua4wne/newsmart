...
@section('content')
    ...
    <ul class="breadcrumb">
        ...
        <li class="active"><a href="{{ route('tarif') }}">{{ $title }}</a></li>
    </ul>
    <!-- END BREADCRUMB -->
    <!-- page content -->
    <div class="row">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        {!! Form::label('koeff', 'Тариф, руб:',['class'=>'col-xs-3 control-label']) !!}
                        <div class="col-xs-9">
                            {!! Form::text('koeff', old('koeff'),['class' => 'form-control','placeholder'=>'Укажите стоимость','required'=>'required','maxlength'=>'9','id'=>'koeff'])!!}
                        </div>
                    </div>

                    <div class="form-group">
                        {!! Form::label('unit', 'Единица учета:',['class'=>'col-xs-3 control-label']) !!}
                        <div class="col-xs-9">
                            {!! Form::text('unit', old('unit'),['class' => 'form-control','placeholder'=>'Укажите единицу учета','required'=>'required','maxlength'=>'5','id'=>'unit'])!!}
                        </div>
                    </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        {!! Form::label('koeff', 'Тариф, руб:',['class'=>'col-xs-3 control-label']) !!}
                        <div class="col-xs-9">
                            {!! Form::text('koeff', old('koeff'),['class' => 'form-control','placeholder'=>'Укажите стоимость','required'=>'required','maxlength'=>'9','id'=>'ekoeff'])!!}
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        {!! Form::label('unit', 'Единица учета:',['class'=>'col-xs-3 control-label']) !!}
                        <div class="col-xs-9">
                            {!! Form::text('unit', old('unit'),['class' => 'form-control','placeholder'=>'Укажите единицу учета','required'=>'required','maxlength'=>'5','id'=>'eunit'])!!}
                        </div>
                    </div>
    ...
    @if($rows)
        <div class="x_content">
            <div class="btn-group">
                <a href="#">
                    <button type="button" class="btn btn-default btn-sm" data-toggle="modal" data-target="#addNew"><i class="fa fa-plus green" aria-hidden="true"></i> Новая единица</button>
                </a>
            </div>
        </div>
...
@section('user_script')
    ...
    <script>
        ...
        $('#edit_btn').click(function(e){
            e.preventDefault();
            var error=0;
            var name = row.prevAll().eq(2).text();
                    ...
                    success: function(res){
                        ...
                        if ($.isEmptyObject(res.error)) {
                            if (res == 'ERR')
                                ...
                                $(".modal").modal("hide");
                                name,
                                row.prevAll().eq(1).text($('#ekoeff').val());
                                row.prevAll().eq(0).text($('#eunit').val());
                            }
                        } else {
                            $(".print-error-msg").find("ul").html('');
                            $(".print-error-msg").css('display', 'block');
        ...
        $(document).on ({
            click: function() {
                ...
                var unit = $(this).parent().parent().prevAll().eq(0).text();
